...
subnet 192.168.1.0 netmask 255.255.255.0 {
    ...
    # --- Dynamic Boot File Logic for UEFI/BIOS ---
    # This logic checks the client's architecture to serve the correct boot file.
    # For UEFI clients, we first check if a MAC-specific GRUB config exists.
    # If it does, we serve that file directly. If not, we serve the default GRUB
    # bootloader, which will then search for its own config.
    if option client-arch = 00:07 or option client-arch = 00:09 {
        # For UEFI clients, serve the GRUB bootloader.
        # GRUB will then automatically search for a MAC-specific config file
        # (e.g., grub.cfg-01-xx-xx-xx-xx-xx-xx) in the TFTP root.
        # If it doesn't find one, it will fall back to the default grub.cfg.
